...
async fn run_mish_state_at_most_once_rhai(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let result = rhai::Engine::new()
            ...
            .register_fn("is_now_between", |timezone: String, start: String, up_to: String| {
...
fn is_now_between(timezone: &str, start: &str, up_to: &str, current_time: chrono::DateTime<chrono::Utc>) -> bool {
    // Parse timezone
...
mod tests {
    ...
    fn test_is_now_between() {
        ...
        assert!(result, "Should be true when current time is between start and up_to");
        ...
        let result = is_now_between(timezone, start, up_to, current_time);
        assert!(result, "Should be true when current time is within midnight-spanning range");
    }
}
