...
impl RingRestClient {
    ...
    pub async fn get_locations(&self) -> String {
        self.request(&format!("{}{}", DEVICE_API_BASE_URL, "locations"))
            .await
    }

    pub async fn get_devices(&self) -> String {
        self.request(&format!("{}{}", CLIENT_API_BASE_URL, "ring_devices"))
            .await
    }
    ...
    pub async fn get_socket_ticket(&self) -> String {
        self.request(&format!(
            "{}{}",
            APP_API_BASE_URL, "clap/ticket/request/signalsocket"
        ))
        .await
    }

    pub async fn get_camera_snapshot(&self) -> axum::body::Bytes {
        let mut map = HashMap::new();
        map.insert("client_id", "ring_official_android");
        ...
        let res = client
            .get(&format!("{}{}", SNAPSHOTS_API_BASE_URL, "next/375458730"))
            .json(&map)
            .header("authorization", auth_value)
            .header("hardware_id", &self.hardware_id)
            .header("User-Agent", "android:com.ringapp")
        ...
        let time_ms = res
            ...
            .parse::<f64>()
            .unwrap();
        let time = DateTime::<Utc>::from_timestamp((time_ms / 1000.) as i64, 0).unwrap();
        println!("time: {time}");
